...
"""
for i in range(0, len(reviews['Reviews'])):
    result = ""
    for res in stem_document(clean_document(sent_tokenize(reviews['Reviews'][i]))):
        result = result + " ".join(res) + ". "
    reviews['Reviews'] = reviews['Reviews'].replace(reviews['Reviews'][i], result)
"""
...
tmp = transformer.get_feature_names()
camera = tmp.index("camera")
cpu = tmp.index("cpu")
display = tmp.index("display")
ram = tmp.index("ram")
...
for brand in z:
    if brand not in brands:
        brands[brand] = {"camera":0, "cpu":0, "display":0, "ram":0}
